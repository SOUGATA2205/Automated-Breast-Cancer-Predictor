...
def send_reset_email(email, token):
    ...
    # Create reset link
    reset_link = f"http://{request.host}/reset-password?token={token}"
    
    # Prepare email
    message = MIMEMultipart()
    message['From'] = f"{sender_name} <{sender_email}>"
    message['To'] = email
    message['Subject'] = "Password Reset Request - Breast Cancer Predictor"
    
    # Email body
    email_body = f"""
    <html>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px; border-left: 5px solid #3498db;">
            <h2 style="color: #2c3e50;">Password Reset Request</h2>
            <p>Hello {user[1]},</p>
            <p>We received a request to reset your password for the Breast Cancer Predictor application.</p>
            <p>To reset your password, please click the button below:</p>
            <p style="text-align: center; margin: 30px 0;">
                <a href="{reset_link}" style="background-color: #3498db; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block;">Reset Password</a>
            </p>
...
def forgot_password():
    if request.method == 'POST':
        ...
        if not user:
            ...
            return render_template('forgot_password.html', success='If your email is registered, you will receive a password reset link shortly.')
...
def reset_password():
    # Get token from query parameters or form
    ...
    if not user_id:
        return render_template('reset_password.html', error='Invalid or expired reset link. Please request a new one.', token=token)
    ...
    if request.method == 'POST':
        ...
        if not password or not confirm_password:
            return render_template('reset_password.html', error='Please enter both password fields', token=token)
        
        if password != confirm_password:
            return render_template('reset_password.html', error='Passwords do not match', token=token)
        
        if not is_strong_password(password):
            return render_template('reset_password.html', error='Password must be at least 8 characters long and include uppercase, lowercase, and numbers', token=token)
        
        # Update password
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute('UPDATE users SET password = ? WHERE id = ?', (generate_password_hash(password), user_id))
        ...
        return redirect(url_for('login', success='Your password has been reset successfully. Please log in with your new password.'))
...
def predict():
    ...
    # Make prediction with feature names to avoid warnings
